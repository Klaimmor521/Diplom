        ...
        if delta > 10:
            with st.container():
                ...
                if target_col == "Количество":
                    text = "**План действий (Ресурсы):**\n"
                    if has_parts or has_goods:
                        text += "- **Запасы:** :green[Увеличьте] объем закупок и сформируйте страховой запас (+20%) по ключевым позициям.\n"
                    if has_accessories:
                        text += "- **Ассортимент:** :green[Расширьте] линейку аксессуаров, особенно высокомаржинальных.\n"
                    if has_services:
                        text += "- **Персонал:** :green[Проверьте] загрузку мастеров и подготовьтесь к росту клиентского потока.\n"
                    st.markdown(text)
                else:  # Выручка
                    text = "**План действий (Финансы и Маркетинг):**\n"
                    text += "- **Инвестиции:** :green[Направьте] растущий денежный поток на закупку оборудования, рекламу или обучение.\n"
                    if has_parts or has_accessories:
                        text += "- **Средний чек:** :green[Предлагайте] более дорогие аналоги и пакетные предложения.\n"
                    if has_services:
                        text += "- **Услуги:** :green[Внедряйте] сопутствующие платные сервисы (например, срочный ремонт).\n"
                    st.markdown(text)
            ...
            with st.container():
                ...
                if target_col == "Количество":
                    text = "**План действий (Ресурсы):**\n"
                    if has_parts or has_goods:
                        text += "- **Закупки:** :red[Приостановите] новые заказы по падающим категориям и сфокусируйтесь на распродаже излишков.\n"
                    if has_accessories:
                        text += "- **Ассортимент:** :red[Сократите] линейку до самых оборачиваемых позиций, запустите акции на неликвиды.\n"
                    if has_services:
                        text += "- **Персонал:** Ожидается снижение загрузки. :red[Запустите] маркетинговые акции (скидки) для привлечения клиентов.\n"
                    st.markdown(text)
                else:  # Выручка
                    text = "**План действий (Финансы и Маркетинг):**\n"
                    text += "- **Расходы:** :red[Проведите] аудит постоянных затрат, сократите неэффективные каналы.\n"
                    if has_parts or has_accessories or has_goods:
                        text += "- **Спрос:** :red[Пересмотрите] ценовую политику, временно снизьте наценку для удержания клиентов.\n"
                    if has_services:
                        text += "- **Лояльность:** :red[Предложите] клиентам программы лояльности или абонементы для удержания базы.\n"
                    st.markdown(text)
            ...
            with st.container():
                ...
                **План действий:**
                ...
                - **Развитие:** :blue[Используйте] стабильный период для улучшения внутренних процессов, обучения сотрудников и внедрения новых стандартов.
                """)
